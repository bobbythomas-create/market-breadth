...
select{border:1px solid var(--rule);background:var(--pnl);color:var(--ink);padding:3px 6px;border-radius:3px;font:inherit;font-size:13px}
...
.obs b{color:var(--dim);font-size:10px;letter-spacing:.11em;text-transform:uppercase;display:block;margin-bottom:2px;font-weight:600}
...
tr.col th{top:19px;font-size:12px;font-weight:600;padding:3px 4px;text-align:right;white-space:nowrap;color:#9fb0bc}
...
.tvb{font-size:13px;padding:4px 11px;border:1px solid #2c6b45;background:#132a1e;color:#6fd39a;border-radius:3px;cursor:pointer;font-weight:600;margin:0 4px 6px 0}
...
.fl{display:inline-block;font-size:8.5px;padding:1px 3px;border-radius:2px;margin-right:2px;
...
.card h3{margin:0 0 8px;font-size:14px;font-weight:650;color:var(--ink);letter-spacing:.02em}
.card .cap{font-size:13px;color:var(--dim);margin-top:6px;line-height:1.5}
...
.gs table{width:100%;font-size:12.5px;font-family:inherit;margin-top:4px;table-layout:fixed}
...
.xchip{font-size:13px;padding:2px 8px;border-radius:10px;font-weight:600;letter-spacing:.02em}
...
.sl{font-size:10px;letter-spacing:.09em;text-transform:uppercase;color:var(--dim);margin-right:3px}
.sb{font-size:13px;padding:3px 9px;border:0;background:transparent;color:var(--dim);cursor:pointer;border-radius:3px;font-weight:600}
.sb.on{background:var(--acc);color:#08110c}.sb:hover:not(.on){color:var(--ink)}
.fno{color:#6f9fd8;font-size:12.5px;margin-left:4px;vertical-align:1px}
...
.cl{font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--dim);margin-right:4px}
.chip2{font-size:13px;padding:2px 9px;border-radius:3px;border:1px solid var(--rule);background:#0d1216;color:#c3ced6}
.chip2 b{font-size:10px;letter-spacing:.05em;margin-right:4px}
.insights{border:1px solid var(--rule);background:var(--pnl);border-radius:3px;padding:8px 12px;margin-bottom:9px}
.ins{font-size:14px;line-height:1.6;color:#c3ced6}
...
.mread{background:#141c22;border:1px solid var(--rule);border-left:3px solid var(--acc);border-radius:3px;
 padding:9px 13px;margin-bottom:9px;font-size:14px;line-height:1.55}
...
.mread .mr-cap{color:var(--dim);font-size:12.5px;margin-top:5px}
.il{font-size:10px;letter-spacing:.1em;font-weight:700;margin-right:8px;display:inline-block;min-width:44px}
.rmd{border-radius:3px;padding:8px 12px;margin-bottom:9px;font-size:14px;line-height:1.5}
.rmd.on{background:#3a1512;border:1.5px solid #c2503c;color:#f0b8ab}
.rmd.on b{color:#ff6b52}
.rmd.soft{background:var(--pnl);border:1px solid var(--rule);color:var(--dim);font-size:12.5px}
.rmd:not(.on):not(.soft){background:#3a2a12;border:1px solid #a07a30;color:#e0c68a}
.rmd code{background:#0d1216;padding:1px 5px;border-radius:2px;font-size:12.5px}
.rbadge{font:12.5px/1 ui-sans-serif;font-weight:700;padding:4px 10px;border-radius:12px;letter-spacing:.03em;white-space:nowrap}
...
.vlabel{font-size:30px;font-weight:800;letter-spacing:-.01em;line-height:1}
.vsub{color:#c3ced6;font-size:14.5px;margin-top:4px}
.vright{text-align:right}
.vreg{font-size:19.5px;font-weight:700}
.vmeta{color:var(--dim);font-size:13px;margin-top:2px;letter-spacing:.03em}
...
.mm.on{display:block}
.mi{padding:7px 13px;font-size:13px;color:var(--dim);cursor:pointer;white-space:nowrap}
...
.sighdr{font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:var(--dim);margin-bottom:4px}
.sigtbl{width:100%;border-collapse:collapse;font-size:12.5px}
...
.chip{border:1px solid var(--rule);border-radius:3px;padding:2px 6px;font-size:13px;background:var(--pnl2)}
.chip b{color:var(--ink)}
.chip i{color:var(--dim);font-style:normal;margin-left:4px;font-size:12px}
...
.lg-i{display:flex;align-items:center;gap:5px;font-size:13px;color:var(--dim)}
...
.ain{font-weight:700;color:var(--ink);font-size:13px;letter-spacing:.02em}
.obar{display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;align-items:center;font-size:14px;margin-bottom:6px}
.ixrow{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px}
.ixchip{border:1px solid var(--rule);border-radius:3px;padding:3px 8px;font-size:13px;background:var(--pnl2);color:var(--dim)}
...
.onote2{font-size:12.5px;color:var(--dim);line-height:1.5;margin-bottom:8px}
...
.ohd{font-weight:700;font-size:14px;color:var(--ink);border-bottom:1px solid var(--rule);padding-bottom:4px;margin-bottom:4px}
.ohd i{font-style:normal;color:var(--dim);font-weight:600}
.onote{font-size:12px;color:var(--dim);margin-bottom:5px}
.oemp{font-size:12.5px;color:var(--dim);padding:6px 0}
.orow{display:grid;grid-template-columns:34px 84px 34px 1fr;gap:5px;align-items:baseline;padding:3px 0;border-bottom:1px solid var(--rule);font-size:12.5px}
.obadge{font-size:9px;font-weight:700;letter-spacing:.03em;padding:1px 3px;border-radius:2px;text-align:center}
...
<div id="tvmsg" style="position:fixed;bottom:16px;left:50%;transform:translateX(-50%);background:#183a26;border:1px solid #2c6b45;color:#6fd39a;padding:8px 16px;border-radius:4px;font-size:14px;opacity:0;transition:opacity .2s;z-index:20;pointer-events:none;max-width:90vw"></div>
...
function traderPane(){const T=TRADER,el=document.getElementById('p-trader');
 ...
 const vixCard=`<div class="card"><h3>India VIX, index options vol</h3>
  <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:baseline;margin-bottom:9px">
   <div><span style="font-size:27.5px;font-weight:800">${v.level??'--'}</span><span class="cap"> VIX level</span></div>
...
function chartPane(){const s0=SER[U],d=DATA[U];if(!s0)return;
 ...
 const bandLbl=[30,45,60].map(v=>{const y=(H-v/100*H).toFixed(1);return `<line x1="0" y1="${y}" x2="${W}" y2="${y}" stroke="#5a6b77" stroke-width=".5" stroke-dasharray="4 4" opacity=".6"/><text x="4" y="${(+y-3).toFixed(1)}" fill="#9fb2be" font-size="11.5">${v}</text>`;}).join('');
 const gridx=[0,.25,.5,.75,1].map(f=>`<line x1="${f*W}" y1="0" x2="${f*W}" y2="${H}" stroke="#2b353e" stroke-width=".6"/>`).join('');
 const lbl=[0,.25,.5,.75,1].map(f=>`<text x="${f*W}" y="${H+13}" fill="#7d8d99" font-size="11.5" text-anchor="${f?f===1?'end':'middle':'start'}">${s.d[Math.round(f*(n-1))]}</text>`).join('');
...
function referencePane(){document.getElementById('p-reference').innerHTML=`
 ...
 <h4 style="margin-top:10px">Watch (high-value videos)</h4>
 <dl>
  <dt>Moving-average breadth, how to read participation (David Keller, StockCharts TV)</dt><dd>youtube.com/watch?v=INF6jADMrGM</dd>
  <dt>Understanding the Stockbee Market Monitor for swing trading</dt><dd>youtube.com/watch?v=XMtlW9ZuU0M</dd>
